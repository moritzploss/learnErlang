...
start_test_() ->
    {"It should be possible to start a pool server and give it a name",
        {setup, fun find_unique_name/0, fun(Name) ->
...
mfa_test_() ->
    {"A pool process can be allocated which will be ordered "
        ...
        {setup, fun start_ppool/0, fun kill_ppool/1, fun(Name) ->
            [pool_run_mfa(Name)]
        end}}.
...
queue_test_() ->
    {"The queue function can be used to run the function as soon as possible. "
        ...
        {foreach, fun start_ppool/0, fun kill_ppool/1, [
...
supervision_test_() ->
    {"The ppool will never restart a dead child, but all children (OTP "
        ...
        "are trapping exits", {setup, fun find_unique_name/0, fun test_supervision/1}}.

auth_test_() ->
    {"The ppool should only dequeue tasks after receiving a down signal "
        "from a worker and nobody else",
        {setup, fun start_ppool/0, fun kill_ppool/1, fun test_auth_dealloc/1}}.
...
pool_run_realloc(Name) ->
    ...
    [
        ?_assert(is_pid(Pid)),
        ?_assertEqual(
            [{A, i_am_running}, {B, i_am_running}, {Pid, i_am_running}],
            L
        )
    ].
